...
public class KartaRestNode implements KartaNode {
    ...
    @Override
    public FeatureResult runFeature(RunInfo runInfo, TestProperties testProperties, TestFeature feature) throws RemoteException {
        FeatureRunInfo featureRunInfo = FeatureRunInfo.builder().runInfo(runInfo).testProperties(testProperties).testFeature(feature).build();

        FeatureResult result = null;

        try {
            RestRequest restRequest = ApacheRestRequest.requestBuilder().header(Constants.ACCEPT, Constants.APPLICATION_JSON).contentType(ContentType.APPLICATION_JSON).body(featureRunInfo).build();
    ...
    public TestJobResult runJobIteration(RunInfo runInfo, String featureName, TestProperties testProperties, TestJob job, int iterationIndex) throws RemoteException {
        JobIterationRunInfo jobIterationRunInfo = JobIterationRunInfo.builder().runInfo(runInfo).featureName(featureName).testProperties(testProperties).testJob(job).iterationIndex(iterationIndex).build();

        TestJobResult result = null;

        try {
            RestRequest restRequest = ApacheRestRequest.requestBuilder().header(Constants.ACCEPT, Constants.APPLICATION_JSON).contentType(ContentType.APPLICATION_JSON).body(jobIterationRunInfo).build();
    ...
    public ScenarioResult runTestScenario(RunInfo runInfo, String featureName, int iterationIndex, PreparedScenario testScenario, long scenarioIterationNumber) throws RemoteException {
        ScenarioRunInfo scenarioRunInfo = ScenarioRunInfo.builder().runInfo(runInfo).featureName(featureName).iterationIndex(iterationIndex).preparedScenario(testScenario).scenarioIterationNumber(scenarioIterationNumber).build();

        ScenarioResult result = null;

        try {
            RestRequest restRequest = ApacheRestRequest.requestBuilder().header(Constants.ACCEPT, Constants.APPLICATION_JSON).contentType(ContentType.APPLICATION_JSON).body(scenarioRunInfo).build();
    ...
    public StepResult runStep(RunInfo runInfo, PreparedStep step) throws RemoteException {
        ...
        try {
            RestRequest restRequest = ApacheRestRequest.requestBuilder().header(Constants.ACCEPT, Constants.APPLICATION_JSON).contentType(ContentType.APPLICATION_JSON).body(stepRunInfo).build();
    ...
    public StepResult performChaosAction(RunInfo runInfo, PreparedChaosAction chaosAction) throws RemoteException {
        ...
        try {
            RestRequest restRequest = ApacheRestRequest.requestBuilder().header(Constants.ACCEPT, Constants.APPLICATION_JSON).contentType(ContentType.APPLICATION_JSON).body(chaosActionRunInfo).build();
    ...
    public boolean healthCheck() throws RemoteException {
        try {
            RestRequest restRequest = ApacheRestRequest.requestBuilder().header(Constants.ACCEPT, Constants.APPLICATION_JSON).contentType(ContentType.APPLICATION_JSON).build();
