...
public class HTMLReportTestEventListener implements TestEventListener
{
         ...
         switch ( event.getEventType() )
         {
            ...
            case StandardEventsTypes.RUN_COMPLETE_EVENT:
               ...
               for ( String testName : keySet )
               {
                  ...
                  runReportBuilder.append( OPEN_TR + SPACE_TAB ).append( tdText ).append( A_HREF ).append( testName ).append( DOT_HTML_CLOSE_TAG )
                           .append( testName ).append( CLOSE_A ).append( CLOSE_TD ).append( SPACE_TAB ).append( tdText )
                           .append( passed ? Constants.PASS : Constants.FAIL ).append( CLOSE_TD ).append( CLOSE_TR );
               }
            ...
            case StandardEventsTypes.JAVA_FEATURE_COMPLETE_EVENT:
               ...
               featureReportBuilder.append(
                        B_INCIDENTS_B + OPEN_TABLE + OPEN_TR + TH_BGCOLOR_BLUE + TAGS + CLOSE_TH + TH_BGCOLOR_BLUE + MESSAGE + CLOSE_TH + TH_BGCOLOR_BLUE + THROWN_CAUSE + CLOSE_TH + TH_BGCOLOR_BLUE + TIME_OF_OCCURRENCE + CLOSE_TH + CLOSE_TR );
               for ( TestIncident incident : featureResult.getIncidents() )
               {
                  featureReportBuilder.append( OPEN_TR + OPEN_TD ).append( incident.getTags() ).append( CLOSE_TD ).append( OPEN_TD )
                           .append( incident.getMessage() ).append( CLOSE_TD ).append( OPEN_TD ).append( incident.getThrownCause() )
                           .append( CLOSE_TD ).append( OPEN_TD ).append( incident.getTimeOfOccurrence() ).append( CLOSE_TD ).append( CLOSE_TR );
               }
               ...
               for ( SerializableKVP<String, StepResult> stepResultPair : featureResult.getSetupResults() )
               {
                  ...
                  String timeTaken = ( stepResult.getStartTime() == null ) || ( stepResult.getEndTime() == null ) ?
                           NA :
                           Duration.between( stepResult.getStartTime().toInstant(), stepResult.getEndTime().toInstant() ).toString();
                  boolean passed = stepResult.isPassed();
                  String  tdText = TD_BGCOLOR + ( passed ? GREEN : RED ) + QUOTE_CLOSE_TAG;
                  featureReportBuilder.append( OPEN_TR + SPACE_TAB ).append( tdText ).append( A_HREF ).append( featureName ).append( Constants.SLASH )
                           .append( Constants.__FEATURE_SETUP__ ).append( Constants.SLASH ).append( stepResult.getStepIndex() )
                           .append( Constants.HYPHEN ).append( stepIdentifier.replaceAll( Constants.REGEX_NON_ALPHANUMERIC, Constants.UNDERSCORE ) )
                           .append( DOT_HTML_CLOSE_TAG ).append( stepIdentifier ).append( CLOSE_A ).append( CLOSE_TD ).append( SPACE_TAB )
                           .append( tdText ).append( timeTaken ).append( CLOSE_TD ).append( SPACE_TAB ).append( tdText )
                           .append( passed ? Constants.PASS : Constants.FAIL ).append( CLOSE_TD ).append( CLOSE_TR );
               }
               featureReportBuilder.append( CLOSE_TABLE );
               ...
               for ( SerializableKVP<String, StepResult> stepResultPair : featureResult.getTearDownResults() )
               {
                  ...
                  String  tdText = TD_BGCOLOR + ( passed ? GREEN : RED ) + QUOTE_CLOSE_TAG;
                  featureReportBuilder.append( OPEN_TR + SPACE_TAB ).append( tdText ).append( A_HREF ).append( featureName ).append( Constants.SLASH )
                           .append( Constants.__FEATURE_TEARDOWN__ ).append( Constants.SLASH ).append( stepResult.getStepIndex() )
                           .append( Constants.HYPHEN ).append( stepIdentifier.replaceAll( Constants.REGEX_NON_ALPHANUMERIC, Constants.UNDERSCORE ) )
                           .append( DOT_HTML_CLOSE_TAG ).append( stepIdentifier ).append( CLOSE_A ).append( CLOSE_TD ).append( SPACE_TAB )
                           .append( tdText ).append( timeTaken ).append( CLOSE_TD ).append( SPACE_TAB ).append( tdText )
                           .append( passed ? Constants.PASS : Constants.FAIL ).append( CLOSE_TD ).append( CLOSE_TR );
            ...
            case StandardEventsTypes.JAVA_SCENARIO_COMPLETE_EVENT:
               ...
               for ( SerializableKVP<String, StepResult> stepResultPair : scenarioResult.getSetupResults() )
               {
                  ...
                  boolean passed = stepResult.isPassed();
                  String  tdText = TD_BGCOLOR + ( passed ? GREEN : RED ) + QUOTE_CLOSE_TAG;
                  scenarioReportBuilder.append( OPEN_TR + SPACE_TAB ).append( tdText ).append( A_HREF ).append( scenarioName ).append( _SETUP )
                           .append( Constants.SLASH ).append( stepResult.getStepIndex() ).append( Constants.HYPHEN )
                           .append( stepIdentifier.replaceAll( Constants.REGEX_NON_ALPHANUMERIC, Constants.UNDERSCORE ) ).append( DOT_HTML_CLOSE_TAG )
                           .append( stepIdentifier ).append( CLOSE_A ).append( CLOSE_TD ).append( SPACE_TAB ).append( tdText ).append( timeTaken )
                           .append( CLOSE_TD ).append( SPACE_TAB ).append( tdText ).append( passed ? Constants.PASS : Constants.FAIL )
               ...
               if ( !scenarioResult.getChaosActionResults().isEmpty() )
               {
                  ...
                  for ( SerializableKVP<String, StepResult> stepResultPair : scenarioResult.getChaosActionResults() )
                  {
                     ...
                     boolean passed = stepResult.isPassed();
                     String  tdText = TD_BGCOLOR + ( passed ? GREEN : RED ) + QUOTE_CLOSE_TAG;
                     scenarioReportBuilder.append( OPEN_TR + SPACE_TAB ).append( tdText ).append( A_HREF ).append( scenarioName ).append( _CHAOS )
                              .append( Constants.SLASH ).append( stepResult.getStepIndex() ).append( Constants.HYPHEN )
                              .append( stepIdentifier.replaceAll( Constants.REGEX_NON_ALPHANUMERIC, Constants.UNDERSCORE ) )
                              .append( DOT_HTML_CLOSE_TAG ).append( stepIdentifier ).append( CLOSE_A ).append( CLOSE_TD ).append( SPACE_TAB )
                              .append( tdText ).append( timeTaken ).append( CLOSE_TD ).append( SPACE_TAB ).append( tdText )
                              .append( passed ? Constants.PASS : Constants.FAIL ).append( CLOSE_TD ).append( CLOSE_TR );
                  }
               ...
               for ( SerializableKVP<String, StepResult> stepResultPair : scenarioResult.getRunResults() )
               {
                  ...
                  boolean passed = stepResult.isPassed();
                  String  tdText = TD_BGCOLOR + ( passed ? GREEN : RED ) + QUOTE_CLOSE_TAG;
                  scenarioReportBuilder.append( OPEN_TR + SPACE_TAB ).append( tdText ).append( A_HREF ).append( scenarioName )
                           .append( Constants.SLASH ).append( stepResult.getStepIndex() ).append( Constants.HYPHEN )
                           .append( stepIdentifier.replaceAll( Constants.REGEX_NON_ALPHANUMERIC, Constants.UNDERSCORE ) ).append( DOT_HTML_CLOSE_TAG )
                           .append( stepIdentifier ).append( CLOSE_A ).append( CLOSE_TD ).append( SPACE_TAB ).append( tdText ).append( timeTaken )
                           .append( CLOSE_TD ).append( SPACE_TAB ).append( tdText ).append( passed ? Constants.PASS : Constants.FAIL )
               ...
               for ( SerializableKVP<String, StepResult> stepResultPair : scenarioResult.getTearDownResults() )
               {
                  ...
                  boolean passed = stepResult.isPassed();
                  String  tdText = TD_BGCOLOR + ( passed ? GREEN : RED ) + QUOTE_CLOSE_TAG;
                  scenarioReportBuilder.append( OPEN_TR + SPACE_TAB ).append( tdText ).append( A_HREF ).append( scenarioName ).append( _TEARDOWN )
                           .append( Constants.SLASH ).append( stepResult.getStepIndex() ).append( Constants.HYPHEN )
                           .append( stepIdentifier.replaceAll( Constants.REGEX_NON_ALPHANUMERIC, Constants.UNDERSCORE ) ).append( DOT_HTML_CLOSE_TAG )
                           .append( stepIdentifier ).append( CLOSE_A ).append( CLOSE_TD ).append( SPACE_TAB ).append( tdText ).append( timeTaken )
                           .append( CLOSE_TD ).append( SPACE_TAB ).append( tdText ).append( passed ? Constants.PASS : Constants.FAIL )
   ...
   public void writeStepReport( String runName, String featureName, long iterationIndex, String scenarioName, StepResult result, String stepId )
            throws IOException
   {
      String iteartionString = "" + ( iterationIndex + 1 );
      Path featureSetupReportDirectory = ( iterationIndex > -1 ) ?
               Paths.get( runReportsBaseFolder.getPath(), runName, featureName, iteartionString, scenarioName ) :
               Paths.get( runReportsBaseFolder.getPath(), runName, featureName, scenarioName );
      Files.createDirectories( featureSetupReportDirectory );
      String fileBaseName = "" + result.getStepIndex() + Constants.HYPHEN + stepId.replaceAll( Constants.REGEX_NON_ALPHANUMERIC,
               Constants.UNDERSCORE );
      Path path = ( iterationIndex > -1 ) ?
               Paths.get( runReportsBaseFolder.getPath(), runName, featureName, iteartionString, scenarioName, fileBaseName + DOT_HTML ) :
               Paths.get( runReportsBaseFolder.getPath(), runName, featureName, scenarioName, fileBaseName + DOT_HTML );
      Path jsonPath = ( iterationIndex > -1 ) ?
               Paths.get( runReportsBaseFolder.getPath(), runName, featureName, iteartionString, scenarioName, fileBaseName + DOT_JSON ) :
      ...
      stepReportBuilder.append(
               B_INCIDENTS_B + OPEN_TABLE + OPEN_TR + TH_BGCOLOR_BLUE + TAGS + CLOSE_TH + TH_BGCOLOR_BLUE + MESSAGE + CLOSE_TH + TH_BGCOLOR_BLUE + THROWN_CAUSE + CLOSE_TH + TH_BGCOLOR_BLUE + TIME_OF_OCCURRENCE + CLOSE_TH + CLOSE_TR );
      for ( TestIncident incident : result.getIncidents() )
      {
         stepReportBuilder.append( OPEN_TR + OPEN_TD ).append( incident.getTags() ).append( CLOSE_TD ).append( OPEN_TD )
                  .append( incident.getMessage() ).append( CLOSE_TD ).append( OPEN_TD ).append( incident.getThrownCause() ).append( CLOSE_TD )
                  .append( OPEN_TD ).append( incident.getTimeOfOccurrence() ).append( CLOSE_TD ).append( CLOSE_TR );
      }
      ...
      {
         stepReportBuilder.append( OPEN_TR + OPEN_TD ).append( resultEntry.getKey() ).append( CLOSE_TD ).append( OPEN_TD )
                  .append( ParserUtils.getObjectMapper().writeValueAsString( resultEntry.getValue() ) ).append( CLOSE_TD ).append( CLOSE_TR );
      }
      stepReportBuilder.append( CLOSE_TABLE );
