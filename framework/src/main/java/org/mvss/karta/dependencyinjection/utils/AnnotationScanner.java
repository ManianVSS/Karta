...
public class AnnotationScanner {
     ...
     */
    public static void forEachMethod(Collection<String> annotationScanPackageNames, Class<? extends Annotation> annotation, Predicate<Integer> modifierChecks, Predicate<Class<?>> returnTypeCheck, Predicate<Parameter[]> paramsChecks, Consumer<Method> action) {
        for (String annotationScanPackageName : annotationScanPackageNames) {
            try {
                Reflections reflections = new Reflections(new ConfigurationBuilder().setUrls(ClasspathHelper.forPackage(annotationScanPackageName)).setScanners(Scanners.MethodsAnnotated));

                Set<Method> candidateMethods = reflections.getMethodsAnnotatedWith(annotation);
                for (Method candidateMethod : candidateMethods) {
                    if (((modifierChecks == null) || modifierChecks.test(candidateMethod.getModifiers())) && ((returnTypeCheck == null) || returnTypeCheck.test(candidateMethod.getReturnType())) && ((paramsChecks == null) || paramsChecks.test(candidateMethod.getParameters()))) {
    ...
    public static void forEachClass(Collection<String> annotationScanPackageNames, Class<? extends Annotation> annotation, Predicate<Integer> modifierChecks, Consumer<Class<?>> action) {
        for (String annotationScanPackageName : annotationScanPackageNames) {
            try {
                Reflections reflections = new Reflections(new ConfigurationBuilder().setUrls(ClasspathHelper.forPackage(annotationScanPackageName)).setScanners(Scanners.TypesAnnotated, Scanners.SubTypes));
     ...
     */
    public static void forEachField(Class<?> classToWorkWith, Class<? extends Annotation> annotation, Predicate<Integer> modifierChecks, AnnotatedFieldConsumer action) {
